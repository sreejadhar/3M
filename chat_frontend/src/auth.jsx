...
// Same JWT scheme as the rest of the stack: token in sessionStorage.auth_token,
// email in localStorage.auth_email; POST /auth/login, GET /auth/validate.
...
// Inject the Bearer token into every same-origin request.
window.fetch = function patchedFetch(url, opts = {}) {
  const token = getToken();
  if (token && typeof url === 'string' && (url.startsWith('/') || url.startsWith(location.origin))) {
    return origFetch(url, { ...opts, headers: { Authorization: `Bearer ${token}`, ...(opts.headers || {}) } });
  }
...
export async function validate() {
  ...
  try {
    const r = await origFetch('/auth/validate', { headers: { Authorization: `Bearer ${token}` } });
