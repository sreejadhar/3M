...
def _llm_fix_sql(
    ...
    system = (
        ...
        "(e.g. TO_DATE, TO_TIMESTAMP): check what the wrapped column actually is per the schema "
        "below. If it is already a DATE/TIMESTAMP-typed column, the cause is a redundant "
        "CAST(... AS DATE) around it — drop the redundant CAST (and the outer TO_DATE call if "
        "the column is already the right type) rather than adding more casts. But if the "
        "column is a NUMBER/decimal holding a large integer (a scaled epoch timestamp, e.g. "
        "EVENT_DATE with values like 1534377600000000), do NOT cast it directly with "
        "CAST(col AS DATE) — that is what caused this error. Instead divide by 1,000,000 to "
        "rescale microseconds to seconds and pass that to TO_DATE: TO_DATE(col / 1000000).\n"
        ...
        "4e. If the error is 'invalid identifier' naming a QUOTED lower-case alias/CTE column "
        "(e.g. invalid identifier 'D.\"cutoff_date\"') and that same name appears elsewhere in "
        "the query defined via an UNQUOTED `AS alias_name` (no double quotes) — that unquoted "
        "definition folded to UPPER CASE, so the later quoted lower-case reference does not "
        "match it. Fix by quoting the alias consistently at its point of definition too, e.g. "
        "change `AS cutoff_date` to `AS \"cutoff_date\"` — do NOT change the reference side "
        "instead, since the query text elsewhere may already rely on the lower-case quoted form.\n"
